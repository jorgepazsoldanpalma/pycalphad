...
def _tdb_grammar(): #pylint: disable=R0914
    ...
    # TODO: need to understand why this change was made
...
def write_tdb(dbf, fd, groupby='subsystem', if_incompatible='warn'):
    ...
    output += ("$" * maxlen) + "\n\n"
    updated_ele=list(dbf.elements)
    updated_ele.append('VA')
    updated_ele.append('/-')    
    for element in sorted(updated_ele):
        if element=='VA':
            ref={'mass': 0.0, 'phase': 'VACUUM', 'H298': 0.0, 'S298': 0.0} 
        elif element=='/-':
            ref={'mass': 0.0, 'phase': 'ELECTRON_GAS', 'H298': 0.0, 'S298': 0.0} 
        else:    
...
####This is where all the species are are listed at the very top of the tdb files
    ...
    for name, phase_obj in sorted(dbf.phases.items()):
        ...
        if len(species_bool)==0:
            full_stoich=[]
            full_stoich_monkey=[]
            species_pure_con=[spec.constituents for i in phase_obj.constituents for spec in i]
            species_pure_con=[i for i in species_pure_con if i not in species_constituents]
            fat_monkey=species_constituents+species_pure_con
            for j in fat_monkey:
                fat_monkey_num=len(j)
                tot_monkey=[]
                for ele,const in j.items():
                    str_ele=str(ele)
                    str_cons=str(const)
                    if fat_monkey_num == 1 and str_cons == '1.0':
                        stoich=str_ele
                    else:
                        stoich=str_ele+str_cons
                    tot_monkey.append(stoich)
                    if len(tot_monkey) ==fat_monkey_num and fat_monkey_num>1:
                        ind=len(full_stoich_monkey)
                        del full_stoich_monkey[ind-1]
                        tot_monkey=''.join(list(sorted(tot_monkey)))
                    elif len(tot_monkey) ==fat_monkey_num and fat_monkey_num==1:
                        tot_monkey=''.join(list(sorted(tot_monkey)))                        
                    full_stoich_monkey.append(tot_monkey)
            full_stoich_monkey = ','.join([spec for spec in full_stoich_monkey])
            txt_len= len(full_stoich_monkey)
#            full_stoich_monkey = fd.write(reflow_text(full_stoich_monkey, linewidth=maxlen))
            if txt_len < 70:
                output += "CONSTITUENT {0} :{1}: !\n".format(name_with_options,full_stoich_monkey)
                output += "\n"
            elif txt_len > 70:
                comma_species_sep = [count for count,i in enumerate(full_stoich_monkey) if i==',']
                distance_to_middle=[i for i in comma_species_sep if (txt_len/2)-i >1]
                distance_to_middle= max(distance_to_middle)
                B = full_stoich_monkey[distance_to_middle+1:]
                C = full_stoich_monkey[:distance_to_middle]
...
######################################### #               
            if param['parameter_type']=='ionic' and param['diffusing_species']==None:
                param['diffusing_species']=Species(None)   
...
################OOOpSS####
        ...
        species_name_add_on_const=[ele.constituents for ele in species if len(ele.constituents)==1 and list(ele.constituents.values())[0] > 1]        
        species_name= species_name+species_name_add_on
        species_const= species_const+species_name_add_on_const
        if constituents in species_name:       
            full_stoich_monkey=[]
            index_spec_name=[count for count,i in enumerate(species_name) if i==constituents]
#            print('sad out loud',species_const[index_spec_name[0]])
            tot_monkey=[]
            for ele,const in species_const[index_spec_name[0]].items():
                str_ele=str(ele)
                str_cons=str(const)
                fat_monkey_num=len(species_const[index_spec_name[0]].items())
                if fat_monkey_num == 1 and str_cons == '1.0':
                    stoich=str_ele
                else:
                    stoich=str_ele+str_cons
                full_stoich_monkey.append(stoich)
                tot_monkey.append(stoich)
